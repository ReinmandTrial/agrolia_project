 ...
 */
    ...
    (() => {
        ...
        if (inputPhones.length) {
            const phoneMask = new Inputmask("+38 (099) 999-99-99", {
                clearIncomplete: true,
                showMaskOnHover: false
            });
            phoneMask.mask(inputPhones);
            modules_flsModules.inputmask = phoneMask;
        }
        const inputLetters = document.querySelectorAll(".input-letters");
        if (inputLetters.length) {
            const lettersMask = new Inputmask({
                regex: "[а-яА-ЯёЁїЇіІєЄa-zA-Z\\s]+",
                placeholder: "",
                showMaskOnHover: false,
                clearIncomplete: true
            });
            lettersMask.mask(inputLetters);
        }
        const inputNumbers = document.querySelectorAll(".input-numbers");
        if (inputNumbers.length) {
            const numbersMask = new Inputmask("9{*}", {
                placeholder: "",
                showMaskOnHover: false,
                clearIncomplete: true
            });
            numbersMask.mask(inputNumbers);
        }
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                ...
                new swiper_core_Swiper(".popup-videos__slider", {
                    modules: [ Pagination ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
        ...
        class AnchorScroll {
            ...
            init() {
                try {
                    ...
                    this.bindEvents();
                    this.log("AnchorScroll initialized successfully");
                } catch (error) {
                    this.error("Failed to initialize AnchorScroll:", error);
                }
            }
            validateEnvironment() {
                if (typeof window === "undefined") throw new Error("AnchorScroll requires browser environment");
                if (!document.querySelector) throw new Error("Browser does not support querySelector");
            }
            cacheElements() {
                this.headerElement = document.querySelector(this.config.headerSelector);
                if (!this.headerElement) this.warn(`Header element not found: ${this.config.headerSelector}`);
                this.links = Array.from(document.querySelectorAll(this.config.linkSelector));
                if (this.links.length === 0) this.warn(`No anchor links found: ${this.config.linkSelector}`);
            }
            calculateHeaderHeight() {
                ...
                this.headerHeight = rect.height + parseFloat(styles.marginTop) + parseFloat(styles.marginBottom);
            }
            bindEvents() {
            ...
            handleLinkClick(event) {
                try {
                    ...
                    if (!this.isValidAnchorLink(href)) return;
                    if (this.config.preventDefault) event.preventDefault();
                    if (isMobile.any && document.documentElement.classList.contains("menu-open")) functions_menuClose();
                    const targetId = this.extractTargetId(href);
                    this.scrollToTarget(targetId, {
                        updateURL: this.config.updateURL,
                        source: "click"
                    });
                } catch (error) {
                    this.error("Error handling link click:", error);
                }
            }
            ...
            findTargetElement(targetId) {
                ...
                try {
                    ...
                    return document.getElementById(targetId) || document.querySelector(`[name="${escapedId}"]`);
                } catch (error) {
                    this.warn(`Invalid target ID: ${targetId}`);
                    return null;
                }
            }
            calculateScrollPosition(element) {
                ...
                return Math.max(0, elementRect.top + scrollTop - this.headerHeight - this.config.offset);
            }
            async scrollToTarget(targetId, options = {}) {
                if (this.isScrolling) {
                    this.warn("Scroll already in progress");
                    return;
                }
                try {
                    const targetElement = this.findTargetElement(targetId);
                    if (!targetElement) {
                        this.warn(`Target element not found: #${targetId}`);
                        return;
                    }
                    const targetPosition = this.calculateScrollPosition(targetElement);
                    if (typeof this.config.onStart === "function") this.config.onStart(targetElement, targetId, options);
                    if (options.updateURL && options.source !== "popstate") this.updateURL(targetId);
                    await this.performScroll(targetPosition);
                    if (typeof this.config.onComplete === "function") this.config.onComplete(targetElement, targetId, options);
                } catch (error) {
                    this.error("Error scrolling to target:", error);
                }
            }
            ...
            updateURL(targetId) {
                ...
                try {
                    ...
                    this.warn("Failed to update URL:", error);
                }
            }
            scrollTo(targetId, options = {}) {
                return this.scrollToTarget(targetId, {
                    updateURL: true,
                    source: "api",
                    ...options
                });
            ...
            destroy() {
                ...
                this.log("AnchorScroll destroyed");
            }
            log(...args) {
                console.log("[AnchorScroll]", ...args);
            }
            warn(...args) {
                console.warn("[AnchorScroll]", ...args);
            }
            error(...args) {
                console.error("[AnchorScroll]", ...args);
            }
        }
        AnchorScroll.create = function(options) {
            return new AnchorScroll(options);
        };
        document.addEventListener("DOMContentLoaded", function() {
            window.anchorScroll = new AnchorScroll({
                headerSelector: ".header",
                linkSelector: ".header ul li a",
                offset: 20,
                duration: 800,
                easing: "easeInOutCubic",
                onStart: (element, targetId) => {
                    console.log(`Scrolling to #${targetId}`);
                },
                onComplete: (element, targetId) => {
                    console.log(`Scrolled to #${targetId}`);
                }
            });
        });
        if (typeof module !== "undefined" && module.exports) module.exports = AnchorScroll;
        function initClientsCarousel() {
            const clientsList = document.querySelector(".clients__list");
            if (!clientsList) {
                console.warn("Элемент .clients__list не найден");
                return;
            }
            clientsList.addEventListener("mouseenter", pauseAnimation);
            clientsList.addEventListener("mouseleave", resumeAnimation);
            clientsList.addEventListener("touchstart", pauseAnimation);
            clientsList.addEventListener("touchend", resumeAnimation);
        ...
        function duplicateClientsItems() {
            const clientsList = document.querySelector(".clients__list");
            const clientsItems = clientsList.querySelectorAll(".clients__item");
            if (!clientsList || clientsItems.length === 0) {
            ...
            const itemWidth = 200;
            const totalWidth = clientsItems.length * 2 * itemWidth;
            clientsList.style.width = `${totalWidth}px`;
        }
        function setCarouselSpeed(duration = 20) {
            const clientsList = document.querySelector(".clients__list");
            if (clientsList) clientsList.style.animationDuration = `${duration}s`;
        }
        document.addEventListener("DOMContentLoaded", function() {
            duplicateClientsItems();
            initClientsCarousel();
        });
